...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    password = serializers.CharField(write_only=True, allow_blank=True, required=False, style={'input_type': 'password'})
    password2 = serializers.CharField(write_only=True, allow_blank=True, required=False, style={'input_type': 'password'})


    class Meta:
        model = User
        fields = ('id', 'username', 'email', 'phone', 'country', 'image', 'city', 'password','password2')
    ...
    def update(self, instance, validated_data):
        ...
        if old_password and new_password:
            if check_password(old_password, instance.password):
                ...
                raise serializers.ValidationError({'password': 'Старый пароль не корректен'})
