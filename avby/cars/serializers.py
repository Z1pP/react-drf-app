...
class CarSerializer(serializers.ModelSerializer):
    ...
    def get_usd_price(self, obj):
        return round(int(obj.price) / 3.46, 2)
...
class CarCreateSerializer(serializers.ModelSerializer):
    ...
    photos = serializers.ListField(
        source="carphoto_set", child=serializers.ImageField()
    )
    ...
    def create(self, validated_data):
        ...
        model_obj, _ = CarModel.objects.get_or_create(
            name=model_name, car_brand_id=brand_obj.id
        )
        seller_obj = User.objects.get(id=seller_id)

        car = Car.objects.create(
            brand=brand_obj, model=model_obj, seller=seller_obj, **validated_data
        )
